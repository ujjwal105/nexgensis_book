...
function BookStripCard({ book }: { book: Book }) {
  return (
    <Link
      to={`/books/${book.id}`}
      className="group flex min-w-[220px] items-center gap-3 rounded-2xl border border-slate-200 bg-white/90 p-3 transition-colors hover:border-slate-300 hover:bg-white dark:border-slate-800 dark:bg-slate-900 dark:hover:border-slate-700"
    >
      <div
...
function SearchResultRow({ book }: { book: Book }) {
  return (
    <Link
      to={`/books/${book.id}`}
      className="group flex items-center gap-4 rounded-2xl border border-transparent px-3 py-3 transition-colors hover:border-slate-200 hover:bg-white/70 dark:hover:border-slate-800 dark:hover:bg-slate-900/70"
    >
      <div
      ...
      <div className="flex justify-center pt-1">
        <div className="relative w-full max-w-[520px]">
          ...
          <Input
            ...
            className="h-14 rounded-full border-slate-300 bg-white/90 pl-12 pr-12 text-lg shadow-[0_16px_40px_rgba(15,23,42,0.08)] dark:border-slate-700 dark:bg-slate-900/90"
            autoFocus
          />
          {searchInput ? (
            <button
              ...
              className="absolute right-4 top-1/2 inline-flex size-7 -translate-y-1/2 items-center justify-center rounded-full text-slate-400 transition-colors hover:bg-slate-100 hover:text-slate-700 dark:hover:bg-slate-800 dark:hover:text-slate-200"
      ...
      {hasQuery ? (
        <div className="space-y-8">
          <section>
            ...
            <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white/70 dark:border-slate-800 dark:bg-slate-900/70">
              {suggestions.length ? (
                suggestions.map((suggestion) => (
                  <button
                    ...
                    className="flex w-full items-center gap-3 border-b border-slate-200/80 px-4 py-4 text-left text-lg text-slate-600 transition-colors last:border-b-0 hover:bg-slate-50 hover:text-slate-900 dark:border-slate-800 dark:text-slate-300 dark:hover:bg-slate-800/70 dark:hover:text-slate-100"
                  >
          ...
          <section>
            ...
            {libraryMatches.length ? (
              ...
              <div className="rounded-3xl border border-dashed border-slate-300 px-5 py-8 text-sm text-slate-500 dark:border-slate-700 dark:text-slate-400">
            ...
            <div className="grid gap-3 md:grid-cols-2">
              {trendingTerms.map((term) => (
                <button
                  ...
                  className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-white/70 px-4 py-4 text-left text-lg text-slate-700 transition-colors hover:border-slate-300 hover:bg-white dark:border-slate-800 dark:bg-slate-900/70 dark:text-slate-300 dark:hover:border-slate-700"
                >
                  <Search className="size-5 flex-none text-slate-400" />
                  <span>{term}</span>
                </button>
